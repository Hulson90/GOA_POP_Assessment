//==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+==+
//
//  Statistical, separable age-structured population (rockfish) model POP GAMMA edition (adds fish selectivity option 2 and 3)                           
...
// Read data from the control file
  !! ad_comm::change_datafile_name("new_mat.ctl");    //Read in phases, penalties and priors from "tem.ctl"
...
PARAMETER_SECTION
 ...
 // Stock-recruitment
  ...
  init_number delta(phase_logist_fsh);                 // age between 50% selection and 95% selection....
   init_number   a502(phase_logist_fsh);                 // age at 50% selection                                                   
  init_number delta2(phase_logist_fsh);                 // age between 50% selection and 95% selection....
  number expa50;
  number expa502;
  ...
  vector 	fish_sel2(1,nages);										       // vectory of fishery selectivty parameters on arithmetic scale
   vector 	fish_sel3(1,nages);										       // vectory of fishery selectivty parameters on arithmetic scale
...
//   Selectivity does not change for ages greater than n_fish_sel_ages
 ...
 if (fsh_sel_opt==1)
  ...
  { 
    if(fsh_sel_opt==3) {
	  for (j=1;j<=nages;j++)  {
     	fish_sel(j)=(pow(j/expa50,expa50/(0.5*(sqrt(square(expa50)+4*square(delta))-expa50)))*mfexp((expa50-j)/(0.5*(sqrt(square(expa50)+4*square(delta))-expa50))));
     	fish_sel2(j)=(pow(j/expa502,expa502/(0.5*(sqrt(square(expa502)+4*square(delta2))-expa502)))*mfexp((expa502-j)/(0.5*(sqrt(square(expa502)+4*square(delta2))-expa502)))); }}
	   else { 
	  	for (j=1;j<=nages;j++)  {
     		fish_sel(j)=(pow(j/expa50,expa50/(0.5*(sqrt(square(expa50)+4*square(delta))-expa50)))*mfexp((expa50-j)/(0.5*(sqrt(square(expa50)+4*square(delta))-expa50))));
       		fish_sel2(j) = 1./(1. + mfexp(-2.944438979*(double(j)-a502)/delta2)); }
            } 
  }
        		fish_sel3 = (fish_sel2 + fish_sel)/2; 
                fish_sel=fish_sel/max(fish_sel);
...
//  Longline survey selectivity set equal to fishery selectivity
  ...
  else //  Estimate survey and fishery selectivity separately
  {
    ...
    {
      for (j=1;j<=nages;j++)
        srv2_sel(j) = 1./(1. + mfexp(-2.944438979*(double(j)-a50_srv2)/delta_srv2));
    }
  }
...
// Calculate mortality rates
  ...
  for (iyr=styr; iyr<=1976; iyr++)
   ...
   for (iyr=1977; iyr<=1995; iyr++)
    F(iyr) = Fmort(iyr) * fish_sel3;		  // Getting fully selected fishing mortality
   for (iyr=1996; iyr<=endyr; iyr++)
    F(iyr) = Fmort(iyr) * fish_sel;		  // Getting fully selected fishing mortality Z = F + natmort;											// Fully selected total mortality
...
// Output file (tem.rep) which is loaded into R for data outputs
  ...
  report << "Age  "<<aa<< endl;
  report << "Fishery_Selectivity 1967-1976" << fish_sel2  <<endl;
  report << "Fishery_Selectivity 1977-1995" << fish_sel3  <<endl;
  report << "Fishery_Selectivity 1996-"<<endyr << fish_sel  <<endl;
...
FUNCTION write_sumreport
  ...
  sumreport << "Age  "<<aa<< endl;
  sumreport << "Fishery_Selectivity 1967-1976" << fish_sel2  <<endl;
  sumreport << "Fishery_Selectivity 1977-1995" << fish_sel3  <<endl;
  sumreport << "Fishery_Selectivity 1996-"<<endyr << fish_sel  <<endl;
